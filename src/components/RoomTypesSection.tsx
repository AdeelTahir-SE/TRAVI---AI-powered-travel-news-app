...
export default function RoomTypesSection({ roomsLink, rooms }: { roomsLink?: string, rooms?: Room[] }) {
  return (
    <section className="min-w-full relative flex flex-col items-center justify-center py-[60px] px-[20px] lg:px-[140px] lg:py-[100px] gap-[80px] ">
      ...
      <div className="flex flex-row items-center justify-center flex-wrap gap-[32px] rounded-[24px] z-10">
        {rooms && rooms?.map((room, i) => {
          return <RoomTypeCard key={i} room={room} />
        })}
...
function RoomTypeCard({ room }: { room: Room }) {
  return (
    <section className="flex flex-col items-start justify-center  p-[32px] gap-[32px] rounded-[30px] border-1 border-[#D0D5DD] bg-white max-w-[530px]">
      <div className="rounded-[24px] gap-[10px]">
        <Image
          src={room?.image ? room?.image : "/images/placeholder.png"}
          width={450}
          height={350}
          className="rounded-[24px] max-h-[355px] w-full h-[355px] gap-[10px] w-full"
          alt=""
        />
      </div>
      ...
      <div className="flex flex-col items-start justify-center gap-[16px]">
        ...
        {room?.link && <Link href={room?.link} className="text-center py-[24px] px-[30px] min-w-full gap-[10px] border-1 border-[#D0D5DD] rounded-[800px] font-inter font-semibold text-[20px] leading-[100%] tracking-[0px]">
          View Rooms Details
        </Link>}
      </div>
    </section>
