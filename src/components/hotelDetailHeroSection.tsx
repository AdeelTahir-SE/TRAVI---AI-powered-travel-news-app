import Link from "next/link";
import CategoryHeroCloudSection from "./categoryHeroCloudSection"
import Image from "next/image"
export default function HotelDetailHeroSection({ title, tagline, hotelImage, checkRatesLink, viewRoomsLink }: { title: string, tagline: string, hotelImage: string, checkRatesLink?: string, viewRoomsLink?: string }) {
  return (
    <div className="flex flex-col items-center justify-center relative w-full  ">
      <section className="flex flex-col items-center justify-center w-full">
        <Image
          src={"https://images.unsplash.com/photo-1566073771259-6a8506099945"}
          width={700}
          height={835}
          className="w-auto object-cover h-[990px] lg:h-[1135px] min-w-full"
          alt=""
        />
        <div className="gap-[24px] lg:gap-[32px] absolute inset-0 w-full h-full flex flex-col items-center justify-center text-white z-10 text-center px-[20px] md:px-[40px] lg:px-[100px] ">
          <div className="flex flex-col items-center justify-center relative gap-[24px] lg:gap-[32px] ">
            <div className="flex flex-col items-center justify-center gap-[12px] z-10">
              <div className="py-[12px] px-[24px] rounded-[600px] bg-[#FFFFFF33] z-10">
                <h2 className="font-inter font-[600px] text-[16px] lg:text-[20px] leading-[30px] tracking-[-0.02em] opacity-100 ">
                  Home &gt; Categories &gt; 5-Star Hotels
                </h2>
              </div>
              <div className="flex flex-row items-center justify-center">
                <h2 className="font-manrope font-extrabold text-[56px]  lg:text-[100px] leading-[68px] lg:leading-[100%] tracking-[-0.03em]">
                  {title},
                  <span className="font-hurricane font-normal text-[74px] lg:text-[160px] leading-[68px] lg:leading-[54.4px] tracking-[-0.03em] text-[#f8a900]">
                    Dubai
                  </span>
                </h2>
              </div>
              <p className="font-inter font-normal text-[20px] lg:text-[24px] leading-[32px]  tracking-[-0.02em] text-center text-[#FFFFFF]">
                {tagline}
              </p>
            </div>
            <div className="flex flex-row items-center justify-center gap-[16px] z-10">
              <Link href={checkRatesLink || "/"} className="py-[24px] px-[30px] bg-[#F8A900] gap-[10px] rounded-[800px] font-inter font-semibold text-[20px] leading-[100%] tracking-[0px] text-black">
                Check Rates
              </Link>
              <Link href={viewRoomsLink || ""} className="py-[24px] px-[30px] border-1 border-[#FFFFFF] gap-[10px] rounded-[800px] font-inter font-semibold text-[20px] leading-[100%] tracking-[0px] text-[#FFFFFF]">
                View Rooms
              </Link>
            </div>
            <div className="absolute inset-0 pointer-events-none z-0 max-h-[350px] bg-[radial-gradient(ellipse_at_center,rgba(35,132,200,0.3)_0%,rgba(35,132,200,0.3)_40%,rgba(35,132,200,0.1)_65%,rgba(35,132,200,0)_100%)] "></div>
          </div>
        </div>
        <div
          className="
              absolute inset-0 z-0
              bg-[linear-gradient(to_bottom,rgba(35,132,200,1)_0%,rgba(35,132,200,0.2)_50%,rgba(35,132,200,0.2)_100%)]
            "
        ></div>
      </section>
      <CategoryHeroCloudSection />
    </div>
  );
}




{/* 
 <div className="flex flex-col items-center justify-center gap-[24px] lg:gap-[32px]">
              <div className="py-[12px] px-[24px] rounded-[600px] bg-[rgba(132,140,151,0.55)] z-10">
                <h2 className="font-inter font-[600px] text-[20px] leading-[30px] tracking-[-0.02em] opacity-100 ">
                  Home &gt; Categories &gt; 5-Star Hotels
                </h2>
              </div>
              <div className="relative flex flex-col items-center justify-center gap-[12px]">
                <div className="absolute inset-0 pointer-events-none z-0 max-h-[350px] bg-[radial-gradient(ellipse_at_center,rgba(35,132,200,0.3)_0%,rgba(35,132,200,0.3)_40%,rgba(35,132,200,0.1)_65%,rgba(35,132,200,0)_100%)]"></div>
                <h1 className="font-manrope font-extrabold text-[56px] lg:text-[100px] leding-[100%] tracking-[-0.03em] z-10">
                  Dubai 5-Star Hotels
                </h1>
                <p className="font-inter font-normal text-[20px] lg:text-[24px] leading-[32px] tracking-[-0.02em] text-center z-10">
                  Explore top-rated luxury hotels in Dubai offering premium
                  amenities, exceptional service, and unforgettable experiences
                  in the heart of the UAE.
                </p>
              </div>

              <div className="flex flex-row items-center justify-center gap-[16px] z-10">
                <button className="py-[24px] px-[30px] bg-[#F8A900] gap-[10px] rounded-[800px] font-inter font-semibold text-[20px] leading-[100%] tracking-[0px] text-black">
                  Check Rates
                </button>
                <button className="py-[24px] px-[30px] border-1 border-[#FFFFFF] gap-[10px] rounded-[800px] font-inter font-semibold text-[20px] leading-[100%] tracking-[0px] text-[#FFFFFF]">
                  View Rooms
                </button>
              </div>
            </div>
          </div> */}